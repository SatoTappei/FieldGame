...
public class AStarTask
{
    ...
    public Stack<Vector3> Execute(AStarNode to, AStarNode from, AStarNode[,] grid)
    {
        BinaryHeap<AStarNode> openHeap = new(MaxPathDistance * 8);
        HashSet<AStarNode> closedSet = new();
        
        openHeap.Add(grid[from.Z, from.X]);

        int count = 0;
        List<AStarNode> neighbourList = new(8);
        while (count++ < MaxPathDistance)
        {
            ...
            // 周囲8つのノードのコストを計算
            foreach (AStarNode neighbour in neighbourList)
            {
                // closeのリストに含まれていたら省く
    ...
    Stack<Vector3> TraceParent(AStarNode to, AStarNode from)
    {
        AStarNode goalNode = to;
    ...
    void GetNeighbour(int z, int x, List<AStarNode> list, AStarNode[,] grid)
    {
        (int i, int k)[] dirs =
        {
            (-1, -1),
